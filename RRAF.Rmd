...
***Figure 1*** *California spiny lobster abundance (counts) at transects within Marine Protected Areas (pink) and at transects not within Marine Protected Areas (blue) between 2012 and 2018. Total lobster counts in 2012: MPA(n) = 32, non-MPA(n) = 199. Total lobster counts in 2018: MPA (n) = 1244, non-MPA(n) = 561.* 
...
In addition to exploring lobster abundance at different sites over time, size differences in lobsters were observed across different sites and years. Figure 2 explores size distribution shifts in 2012 and 2018 across the five sites. 
...
ggplot(lobster_size, aes(x = factor(year), y = size_mm)) +
  ...
  ggtitle(label = "CALIFORNIA SPINY LOBSTER SIZE DISTRIBUTION (2012 & 2018)",
          subtitle = "Distirbution of size(mm) at all sites")


ggplot(lobster_size, aes(x = factor(year), y = size_mm)) +
  geom_jitter(aes(color = site_type),
              alpha = 0.7)+
  geom_boxplot(width = 0.3)+
  scale_color_manual(values = color_group)+
  theme_light() +
  facet_grid(~site) +
  # theme(strip.background = element_rect(fill = c("blue", "blue", "green", "green", "green")))+
  labs(x = "Site",
       y = "Size (mm)",
       color = "Site Type")+
  ggtitle(label = "CALIFORNIA SPINY LOBSTER SIZE DISTRIBUTION (2012 & 2018)",
...
Summary statistics were calculated for lobster size in different protection status sites in 2012 and 2018. Mean lobster size (mm) was compared and analyzed using two-sample t-tests for four groupings: MPA and non-MPA sites in 2012, MPA and non-MPA sites in 2018, MPA sites in 2012 and 2018, and non-MPA sites in 2012 and 2018.


***Table 1:*** *Lobster size distribution in 2012 and 2018 in MPA and non-MPA sites. Mean size, standard deviation, and sample size are compared between years and protection status.*
...
* Abundance of lobsters in both MPA and non-MPA sites increased between 2012 and 2018, with the most dramatic increase observed in Isla Vista. Lobster abundance at MPA sites increased by 3,887.5% between 2012 and 2018, compared to an increase of only 281.9% in non-MPA sites (Figure 1).
